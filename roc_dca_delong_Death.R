...
output_dir <- file.path(base, "Death_ROC_DCA_DeLong")
...
write_csv(
  comparison,
  file.path(output_dir, "OOF_predictions_all_models.csv")
)
...
write_csv(auc_table, file.path(output_dir, "AUC_95CI.csv"))
...
write_csv(delong_table, file.path(output_dir, "DeLong_tests.csv"))
...
save_plot(p_roc, "ROC_Death_95CI_ribbon")
...
save_plot(p_dca, "Decision_Curve_Death_final")

write_csv(
  as_tibble(dca_result),
  file.path(output_dir, "Decision_Curve_net_benefit.csv")
)
...
cat("\n========================================\n")
cat("MORTALITY ANALYSIS COMPLETE\n")
cat("========================================\n")

cat(
  "\nROC with 95% CI ribbon:\n",
  file.path(output_dir, "ROC_Death_95CI_ribbon.pdf"),
  "\n"
)
cat(
  "\nDecision curve:\n",
  file.path(output_dir, "Decision_Curve_Death_final.pdf"),
  "\n"
)
