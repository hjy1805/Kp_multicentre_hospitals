...
# ============================================================
# 3. PREPARE MORTALITY DATA
# ============================================================

mortality_data <- kleborate_encoded_df %>%
  mutate(
    across(c(Mortality, AGE, BMI, CHARLSON), as.numeric),
...
# ============================================================
# 5. ADJUSTED MORTALITY MODEL FOR ONE DETERMINANT
# ============================================================

run_mortality_model <- function(gene) {
  model_df <- mortality_data %>%
    select(Mortality, all_of(gene), all_of(COVARIATES)) %>%
  ...
  # Outcome must vary
  if (length(unique(model_df$Mortality)) < 2) {
  ...
  fit <- tryCatch(
    suppressWarnings(
      glm(
        reformulate(c(gene, COVARIATES), response = "Mortality"),
        data = model_df,
        family = binomial()
      )
    ),
...
# ============================================================
# 6. RUN ALL MORTALITY MODELS
# ============================================================

mortality_all <- map_dfr(determinants, run_mortality_model)
...
cat("\nALL DETERMINANTS:\n")
print(select(mortality_all, all_of(SUMMARY_COLS)))
...
included_order <- determinants[determinants %in% mortality_all$Gene[
  mortality_all$Status == "Included"
]]

plot_order <- gene_labels[included_order]

mortality_results <- mortality_all %>%
  filter(Status == "Included") %>%
  mutate(
...
# ============================================================
# 8. MORTALITY FOREST PLOT
# ============================================================

SIG_COLOURS <- c("FALSE" = "#4C78A8", "TRUE" = "red")

p_mortality <- ggplot(mortality_results, aes(x = estimate, y = Label)) +
  # OR = 1
  geom_vline(
    xintercept = 1,
    linetype = "dashed",
